...
module setPME
  ...
  integer, parameter :: kmax1 = 128
  integer, parameter :: kmax2 = 128
  integer, parameter :: kmax3 = 128
...
program main
  ...
  real(8), parameter :: pi = acos(-1.0d0)
    ...
    call fr_pmel_p3(invlx, invly, invlz, r4pie, rx, ry, rz, q, &
         key1,key2,key3,ww1,ww2,ww3, bscx,bscy,bscz,&
         eng_io, fx, fy, fz )

    open(34,file="EnergyF90.dat")
        write(34,*) eng_io, "10J/mol"
    close(34)

    open(33,file="ForceF90.dat")
...
subroutine fr_pmel_p3(invlx, invly, invlz, r4pie, rx, ry, rz, q,&
           key1,key2,key3,ww1,ww2,ww3,bscx,bscy,bscz, &
           eng_io, fx, fy, fz )
  use Box
  use setPME
  ...
  do i=1, natm
    tmp=rx(i) - anint(rx(i)*invlx)*lx; tx(i)=dble(kmax1)*(invlx*tmp+0.5d0)
    tmp=ry(i) - anint(ry(i)*invly)*ly; ty(i)=dble(kmax2)*(invly*tmp+0.5d0)
    tmp=rz(i) - anint(rz(i)*invlz)*lz; tz(i)=dble(kmax3)*(invlz*tmp+0.5d0)
  end do
    ...
    do k=1, kmax2
      ...
      do j=1, kmax1
        ...
        if (rksq.gt.1.d-6) then
          ...
          !   F^-1 [Q] = qqq(j,k,l)
          !
          ! see page 8581: the transformed Q array is overwritten by the product
          ! of itself
          ! with the arrays C and B defined in Eq. (3.8) and Eq. (4.8)
          qqq(j,k,l)=bb1*exp(r4alpsq*rksq)/rksq*qqq(j,k,l)
        else
          qqq(j,k,l)=(0.d0,0.d0)
        end if
      end do
  ...
  cpetot = sum(qqq(:,:,:)*qqc(:,:,:))
  engcoeffi = tpi/(lx*ly*lz)*r4pie; eng_io = eng_io + real(cpetot)*engcoeffi
