...
void calc_interaction_mesh(double& lX,
                    ...
                    std::complex<double>>>>& interaction_mesh){
    ...
    std::vector<int> pkey1(pKmax1);
    std::vector<int> pkey2(pKmax2);
    std::vector<int> pkey3(pKmax3);
    std::vector<std::complex<double>> pww1(pKmax1);
    std::vector<std::complex<double>> pww2(pKmax2);
    std::vector<std::complex<double>> pww3(pKmax3);
    calc_root_unity__bit_address(pKmax1, pKmax2, pKmax3,
                                 pkey1, pkey2, pkey3,
                                 pww1, pww2, pww3);

    std::vector<std::vector<std::vector<std::complex<double>>>>
    interaction_dense(pKmax1, std::vector<std::vector<std::complex<double>>>
    ...
    double pi = acos(-1.e0);
    double twopi = 2.e0*pi;
    double r4alpsq = -0.25e0/(alpha*alpha);
    double scale = 2.e0*twopi*invlX*invlY*invlZ; 
    double ksq, kX, kY, kZ;

    for (int k3 = 0; k3 < pKmax3; ++k3 ){
        int kk3 = k3; if ( k3 > pKmax3/2 ){ kk3 = k3-pKmax3; };   
        kZ=twopi*double(kk3)*invlZ;

        for (int k2 = 0; k2 < pKmax2; ++k2 ){
	    int kk2 = k2; if ( k2 > pKmax2/2 ){ kk2 = k2-pKmax2; };
            kY=twopi*double(kk2)*invlY;
        
            for (int k1 = 0; k1 < pKmax1; ++k1 ){
                int kk1 = k1; if ( k1 > pKmax1/2 ){ kk1 = k1-pKmax1; };
                kX=twopi*double(kk1)*invlX;

                ksq = kX*kX + kY*kY + kZ*kZ;

                if ( ksq > 1.0e-6 ){
    	            interaction_dense[k1][k2][k3] = {expf(r4alpsq*ksq)/ksq*scale,0.e0};
                }
		        else{
		            interaction_dense[k1][k2][k3] = {0.e0,0.e0};
	   	       }
    }   }   }


    bool is_negetive = false;
    Fast_Fourier_Transform_3D(pKmax1, pKmax2, pKmax3,
                              pkey1,pkey2,pkey3,
                              pww1,pww2,pww3,
                              interaction_dense,
                    	      is_negetive );
    ...
    int multiple1 = pKmax1/Kmax1;
    int multiple2 = pKmax2/Kmax2;
    int multiple3 = pKmax3/Kmax3;
  
    scale = 1.e0/(Kmax1*Kmax2*Kmax3);

    for (int n3 = 0; n3 < Kmax3; ++n3 ){
        int nn3 = n3*multiple3;

        for (int n2 = 0; n2 < Kmax2; ++n2 ){
	        int nn2 = n2*multiple2;

            for (int n1 = 0; n1 < Kmax1; ++n1 ){
	            int nn1 = n1*multiple1;

                interaction_mesh[n1][n2][n3] = interaction_dense[nn1][nn2][nn3]*scale;
    }   }   }
...
void calc_recSpace_Energy_Force(int& npar,
    ...
    */ 
    ...
    charge_grid(Kmax3, std::vector<std::vector<double>>
    ...
    */ 
    ...
    std::vector<std::vector<std::vector<std::complex<double>>>>
        charge_grid_complex( Kmax3, std::vector<std::vector<std::complex<double>>>
    ...
    */
    ...
    double non_neutral_baseline = 0.e0;
    ...
    for (int m3 = 0; m3 < Kmax3; ++m3 ){
        ...
        for (int m2 = 0; m2 < Kmax2; ++m2 ){
            ...
            for (int m1 = 0; m1 < Kmax1; ++m1 ){
                ...
                charge_grid_complex[m1][m2][m3] = B*interaction_mesh[m1][m2][m3]
                                                  *charge_grid_complex[m1][m2][m3];  

                non_neutral_baseline += interaction_mesh[m1][m2][m3].real();
    }   }   }
    ...
    */ 
    ...
    for (int m1 = 0; m1 < Kmax1; ++m1 ){
        for (int m2 = 0; m2 < Kmax2; ++m2 ){
            for (int m3 = 0; m3 < Kmax3; ++m3 ){
                E_complex = E_complex + charge_grid[m1][m2][m3]
                                        *charge_grid_complex[m1][m2][m3];
    }   }   }
    ...
    non_neutral_baseline = non_neutral_baseline*qtotsq;

    double E_coeffs = 0.5e0*r4pie;

    E = E_coeffs*( E_complex.real() - non_neutral_baseline );
                    ...
                    if ( m2 < 0 ){ m2 = m2 + Kmax2; }
                        ...
                        for (int l = 0; l < n_order; ++l ){
                            ...
                            if ( m1 < 0 ){ m1 = m1 + Kmax1; }
                                ...
                                ZZUR = charge_grid_complex[m1][m2][m3].real();
...
int main(){
    ...
    std::vector<std::vector<std::vector<std::complex<double>>>>
        interaction_mesh(Kmax3, std::vector<std::vector<std::complex<double>>>
                        (Kmax2, std::vector<std::complex<double>>
                        (Kmax1) ) );
    ...
    std::ofstream dat_O("Forcecpp.dat");
    dat_O.precision(16);
