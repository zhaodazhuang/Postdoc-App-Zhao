...
__global__ void compute_interaction_mesh_dense_gpu(int pKmax1,
    ...
    int tid = blockIdx.x * blockDim.x + threadIdx.x;
    int total = pKmax1 * pKmax2 * pKmax3;
    ...
    if (tid < total) {
        ...
        if ( m3 > pKmax3/2 ){ m3 = m3-pKmax3; };
            double kZ = twopi*double(m3)*invlZ;
        ...
        if ( m1 > pKmax1/2 ){ m1 = m1-pKmax1; };
            ...
            if ( k > 1.e-6 ){
                ...
                d_interaction_mesh_dense[tid].x = exp(-0.25e0*ksq/(alpha*alpha))/ksq*scalor;
...
__global__ void  extract_interaction_mesh(int pKmax1,
	...
	int m3 = tid / (Kmax1*Kmax2);
        ...
        int id_dense = m3*multi3*(pKmax1*pKmax2) + m2*multi2*pKmax1 + m1*multi1;
...
void calc_interaction_mesh(double lX,
    ...
    cufftDoubleComplex *d_interaction_mesh_dense;
    cudaMalloc(&d_interaction_mesh_dense, pKmax1*pKmax2*pKmax3*sizeof(cufftDoubleComplex));

    double invlX = 1.e0/lX;
    double invlY = 1.e0/lY;
    double invlZ = 1.e0/lZ;
    double twopi = 2.e0*acos(-1.e0);
    double scalor = 2.e0*twopi*invlX*invlY*invlZ;

    compute_interaction_mesh_dense_gpu<<<(pKmax1*pKmax2*pKmax3+511)/512,512>>>(pKmax1, pKmax2, pKmax3,
		                                                              invlX, invlY, invlZ,
		                                                              alpha, twopi, scalor,
    ...
    cufftHandle plan_dense;
    cufftPlan3d(&plan_dense, pKmax3, pKmax2, pKmax1, CUFFT_Z2Z);

    cufftExecZ2Z(plan_dense, d_interaction_mesh_dense, d_interaction_mesh_dense, CUFFT_INVERSE);

    int multi1 = pKmax1/Kmax1;
    int multi2 = pKmax2/Kmax2;
    int multi3 = pKmax3/Kmax3; 

    double coeffi = 1.e0/double(Kmax1*Kmax2*Kmax3);

    extract_interaction_mesh<<<(Kmax1*Kmax2*Kmax3+255)/256, 256>>>(pKmax1, pKmax2, pKmax3,
		                                                     Kmax1, Kmax2, Kmax3,
...
__global__ void  compute_frac_coor(int npar,
	...
	double uu = d_rX[i] - my_round(d_rX[i]/lX)*lX;
        ...
        uu = d_rZ[i]- my_round(d_rZ[i]/lZ)*lZ;
	d_uZ[i] = double(Kmax3)*(uu/lZ+0.5e0);    	
    }
}
...
__global__ void sum_real_reduction(int N,
                                   cufftDoubleComplex* d_data,
                                   double* d_result) {
    // Each thread accumulates its own portion of the sum.
    // Example (N=64, 8 threads):
    // Thread 0: d_data[0] + d_data[8] + ... + d_data[56] (8 elements)
    // Thread 1: d_data[1] + d_data[9] + ... + d_data[57] (8 elements)
    // ...
    // Thread 7: d_data[7] + d_data[15] + ... + d_data[63] (8 elements)
    // Each thread now holds a partial sum (8 values each).

    __shared__ double s_partial[256]; // Shared mem for partial sums (1 per thread)
    int tid = threadIdx.x;          // Thread ID within the block (0-255 for 256 threads)
    int i = blockIdx.x * blockDim.x + tid; // Global thread index
    int stride = gridDim.x * blockDim.x;   // Total threads in the grid
    double thread_sum = 0.0;

    // Strided accumulation: Each thread jumps by total threads to cover all N elements.
    while (i < N) {
        thread_sum += d_data[i].x; // .x is real part
        i += stride;
    }

    s_partial[tid] = thread_sum; // Store partial sum in shared memory
    __syncthreads();             // Wait for all threads in the block to finish

    // Reduction: Combine partial sums into 1.
    // Step 1 (stride=128): s_partial[0]+=s_partial[128], [1]+=[129], ..., [127]+=[255]
    // Step 2 (stride=64): s_partial[0]+=s_partial[64], [1]+=[65], ..., [63]+=[127]
    // ... continue halving until stride=1
    ...
    // Thread 0 writes the block's total sum to global result
    if (tid == 0) {
        atomicAdd(d_result, s_partial[0]);
    }
}

__global__ void sum_reduction(int N, 
		              double* d_vector, 
			      double* d_result) {
    int tid = threadIdx.x;
...
void calc_recSpace_Energy_Force(int npar,
    ...
    */ 
    ...
    double *d_non_neutral_beasline;
    cudaMalloc(&d_non_neutral_beasline, sizeof(double));
    cudaMemset(d_non_neutral_beasline, 0, sizeof(double));

    sum_real_reduction<<<(Kmax1*Kmax2*Kmax3+255)/256, 256>>>(Kmax1*Kmax2*Kmax3,
                                                             d_interaction_mesh,
                                                             d_non_neutral_beasline);

    double non_neutral_beasline;
    cudaMemcpy(&non_neutral_beasline, d_non_neutral_beasline, sizeof(double), cudaMemcpyDeviceToHost);

    cudaFree(d_non_neutral_beasline);
    ...
    */ 
    ...
    double *d_qtot;
    cudaMalloc(&d_qtot, sizeof(double));
    cudaMemset(d_qtot, 0, sizeof(double));

    sum_reduction<<<(npar+255)/256, 256>>>(npar,
                                           d_q,
                                           d_qtot);
    double qtotsq;
    cudaMemcpy(&qtotsq, d_qtot, sizeof(double), cudaMemcpyDeviceToHost);
    qtotsq = qtotsq*qtotsq; 

    E = 0.5 * r4pie *( dot_result - non_neutral_beasline*qtotsq );
...
//////////////////////////////////////////////////
    ...
    int n_order = 6; // interpolation order 
    int Kmax1 = 32;
    int Kmax2 = 32;
    int Kmax3 = 32; // PME mesh size 
    ...
    std::ofstream dat_O("Forcecu.dat");
    dat_O.precision(16);
